...
readonly VLLM=${AI_SERVER_PHASE_J_VLLM:-/home/michel/LLMs-tests/.phase-f1p5-vllm-nightly/bin/vllm}
readonly ALLOWED_GPU0_PID=${AI_SERVER_PHASE_D_ALLOWED_GPU0_PID:?set the approved unrelated GPU 0 compute PID}
readonly REQUEST_TIMEOUT=${AI_SERVER_PHASE_J_REQUEST_TIMEOUT:-600}
...
if ! [[ ${REQUEST_TIMEOUT} =~ ^[1-9][0-9]*$ ]] || [[ ${REQUEST_TIMEOUT} -gt 600 ]] || [[ ! -x ${RUNNER} || ! -x ${VLLM} || ! -f ${MODEL}/config.json ]]; then
    printf '%s\n' "Phase J timeout, runner, nightly vLLM, or calibrated model is not usable" >&2
    exit 1
fi

# The fourth long request stays in the harness until C=0; do not configure a vLLM internal queue here.
AI_SERVER_PHASE_D_ALLOWED_GPU0_PID=${ALLOWED_GPU0_PID} \
AI_SERVER_PHASE_D_MODEL=${MODEL} \
AI_SERVER_PHASE_D_RUN_PREFIX="phase-j-graph-b0-cached-c3-client-queued-cold" \
